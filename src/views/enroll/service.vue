<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.title" :placeholder="$t('table.fullname')" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column :label="$t('table.actions')" align="center" width="230" class-name="small-padding fixed-width">
        <template slot-scope="{row,$index}">
          ...
          <el-button v-if="row.status!='待审'" size="mini" type="success" @click="handleModifyStatus(row,'审核通过')">
            {{ $t('table.publish') }}
          </el-button>
          <el-button v-if="row.status!='审核通过'" size="mini" @click="handleModifyStatus(row,'审核未通过')">
            {{ $t('table.draft') }}
          </el-button>
